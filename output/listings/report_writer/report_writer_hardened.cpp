...
FixedString<50> charge_detail;
FixedString<50> end_of_file_switch;
FixedString<50> page_head_group;
int RETURN_CODE = 0;
...
void p_000_initiate() {
    ...
    if (file_status != "EXCEPTION_0") {
        if (file_status == "AT_END") {
            end_of_file_switch.at(0) = 'Y';
        } else {
        }
    }
    while (true) {
        if (end_of_file_switch.at(0) == 'Y') break;
