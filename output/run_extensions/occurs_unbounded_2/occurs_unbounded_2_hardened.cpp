...
FixedString<50> buffer;
FixedString<15> num_elements;
int RETURN_CODE = 0;
FixedString<50> size_needed;
FixedString<50> vargrp;
FixedString<50> vptr;
FixedString<50> X;
...
FixedString<50> old_size;

// Forward declarations
void p_alloc_vargrp();

void p_alloc_vargrp() {
    num_elements = to_string(to_num(0) + to_num(1));
    p_alloc_vargrp();
    // UNHANDLED: memcpy (b_22 + 22 * 16, "HI MOM  ", 8);
    std::cout << "VARTAB(12) = \\" << "\\" << std::endl;
    std::cout << "VARTAB(17) = \\" << "\\" << std::endl;
    num_elements = to_string(to_num(0) + to_num(1));
    p_alloc_vargrp();
    std::cout << "VARTAB(12) = \\" << "\\" << std::endl;
    std::cout << "VARTAB(17) = \\" << "\\" << std::endl;
    if (false /* TODO: ((*(unsigned char **) (vptr)) - (cob_u8_ptr)nullptr) == 0 */) {
        size_needed = to_string(to_num(0) + to_num(1));
        std::cout << "First allocation, using " << size_needed << " bytes." << std::endl;
        // UNHANDLED: cob_allocate (nullptr, &f_21, &f_19, &cob_all_low);
        vargrp = num_elements.substr(0, 2);
        old_size = size_needed.substr(0, 2);
    } else {
        size_needed = to_string(to_num(0) + to_num(1));
        std::cout << "Re-allocation, using " << size_needed << " bytes." << std::endl;
        if (to_int(RETURN_CODE) < 2097152) {
            // UNHANDLED: cob_allocate (nullptr, &f_21, &f_19, &cob_all_low);
        } else {
            // UNHANDLED: cob_allocate (nullptr, &f_21, &f_19, &cob_all_low);
        }
        ...
        old_size = size_needed.substr(0, 2);
        // UNHANDLED: cob_free_alloc (nullptr, b_21);
    }
}
