...
FixedString<20> altkey;
FixedString<50> display_record;
FixedString<1> duplicate_flag;
FixedString<50> indexing_record;
FixedString<2> indexing_status;
FixedString<2> indexing_status_code;
FixedString<20> keyfield;
FixedString<20> oldkey;
FixedString<1> record_flag;
FixedString<50> safety_net;
...
void P_read_indexing_record() {
    ...
    if (file_status != "EXCEPTION_0") {
        if (file_status == "EXCEPTION_0x0506") {
            ...
            duplicate_flag.at(0) = static_cast<char>(255);
        } else {
        }
    }
    P_indexing_check();
...
void P_read_next_record() {
    ...
    display_record.at(17) = altkey.at(11);
...
void P_start_prime_key() {
    ...
    // UNHANDLED: cob_start (h_INDEXED_FILE, 2, &f_18, nullptr, &f_27);
    if (file_status != "EXCEPTION_0") {
        if (file_status == "EXCEPTION_0x0506") {
            std::cout << "bad start: " << keyfield << std::endl;
            record_flag.at(0) = static_cast<char>(255);
        } else {
        }
    } else {
        // READ NEXT INDEXED-FILE
...
void P_start_at_key() {
    ...
    // UNHANDLED: cob_start (h_INDEXED_FILE, 5, &f_20, nullptr, &f_27);
    if (file_status != "EXCEPTION_0") {
        if (file_status == "EXCEPTION_0x0506") {
